...
# write.table(Building_data_filtered, file="Building_data_filtered.csv", sep=",", row.names = FALSE)

Building_data_filtered<-read.csv("Building_data_filtered.csv")

# The negative ECM values are there because sometimes the ECM increases the energy consumption
Building_data_ecm_corrected<- Building_data_filtered |>
  group_by(Timestamp,Year, Date, Hour)|>
  summarise(Date=ymd(Date),
            ECM_Cooling_kWh=sum(EE_Cooling_kWh, na.rm=T),
            ECM_Heating_mmBTU=sum(EE_Heating_mmBTU, na.rm=T))

summary(Building_data_ecm_corrected)
...
#Capacity
capacity_shed <- read.csv("loads_shed_estimates_CC_2019_22.csv")
...
capacity_shed<-capacity_shed|>
  ...
  dplyr::select(-Year,-towt_elec, -towt_cw,- Elec_Fixed_Price_MW)#,-Elec_Same_Day,-CW_Same_Day

#Energy LMP is $/MWh
energy_shed <- read.csv("loads_shed_Energy_Market.csv")
energy_shed<-energy_shed|> mutate(Market="Energy")|> rename(Price=total_lmp_rt) 

#SR
SR_shed <- read.csv("loads_shed_SR.csv")
SR_shed<-SR_shed|> mutate(Market="SR")|>rename(Price=mcp_capped)
...
# peak<-data_2019_22|>
#   dplyr::group_by(Year, Timestamp)|>
#   summarise(psum=sum(Electricity_Consumed_kWh))|>
#   group_by(Year)|>
#   summarise(psum=max(psum))
#breakdown
...
# This is where the grid compensation for capacity market happens
avoided_peak_S1<-avoided_peak_S1|>
  mutate(B_cap1=delta_P/1000*Price*365 ,
         B_cap=B_cap1*0.8, 
         B_peak=delta_P/1000*Elec_Fixed_Price_MW, 
         Market="BAU:Capacity-DR",
         B_cap2=delta_P/1000*329.17*365*0.8) # 2025 Price 269.92
...
avoided_peak_S2<-avoided_peak_S2|>
  mutate(B_cap1=delta_P/1000*Price*365 ,
         ...
         B_peak=delta_P/1000*Elec_Fixed_Price_MW,
         Market="S1:Capacity",
         B_cap2=delta_P/1000*269.92*365*0.8) # 2025 Price
...
avoided_peak_S5<-avoided_peak_S5|>
  mutate(B_cap1=delta_P/1000*Price*365 ,
         B_cap=B_cap1*0.8, # Cut for CSP  # Need to think about this way of calculating avoided peak for capacity market
         B_peak=delta_P/1000*Elec_Fixed_Price_MW,
         Market="S4:Combined",
         B_cap2=delta_P/1000*269.92*365*0.8) # 2025 Price
...
write.table(avoided_peak_summary, file="avoided_peak_summary.csv", sep=",", row.names = FALSE)
save.image("C:/Users/malhotra.164/OneDrive - The Ohio State University/Yashvi Malhotra-KWEST/BCA/R_BCA/R_BCA/Raw_data/Images/Avoided_Peak.RData")
