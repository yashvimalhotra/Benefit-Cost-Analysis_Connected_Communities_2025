...
# Quant data -- for other markets --DR dates for energy  from 2019 to 2022

lmp_weather <- read.csv("C:/Users/malhotra.164/OneDrive - The Ohio State University/Yashvi Malhotra-KWEST/BCA/R_BCA/R_BCA/Raw_data/lmp_weather.csv")
...
LMP_summary <- lmp_weather |>
  filter(Year==2022)|>
  group_by(Date,Hour) |>
  summarise(
    Days_LMP_over_150 = sum(total_lmp_rt > 150),
    Days_LMP_over_200 = sum(total_lmp_rt > 200),
    Days_LMP_over_300 = sum(total_lmp_rt > 300),
    .groups = "drop"
  ) |>
  filter(Days_LMP_over_150 > 0 | Days_LMP_over_200 > 0 | Days_LMP_over_300 > 0)

LMP_events <-lmp_weather |>
  filter(total_lmp_rt>=200)|> ## Scenario
  # filter(total_lmp_rt>=300)|> ## Scenario
  mutate(event_days = ymd(Date),
         tentative_end_date = event_days - days(1),
         end_date = prev_business_day(tentative_end_date)) |>
  group_by(event_days) |> 
  mutate(starthour = min(Hour), endhour = max(Hour),  
    DR_preevent = if_else(starthour - 1 <= 0, starthour, starthour - 1)) |>
  ungroup() |>
  dplyr::select(Date,event_days,Hour, end_date, starthour, endhour, DR_preevent,total_lmp_rt)

events_join<-LMP_events|> mutate(Date= ymd(Date),DR.Event="Yes")|>
  dplyr::select(Date,Hour,DR.Event,total_lmp_rt)|>distinct()

# Step 2: Create all combinations of dates and hours
all_hours <- expand.grid(Date = unique(events_join$Date), Hour = 0:23)

# Step 3: Join back to the original data
LMP_events_join <- all_hours|>
  left_join(events_join, by = c("Date", "Hour")) |>
  arrange(Hour,Date)|>
  mutate(DR.Day="Yes")
...
Building_data<- left_join(Building_data,LMP_events_join,relationship = "many-to-many")
Building_data<- Building_data |>
  mutate(DR.Day=ifelse(is.na(DR.Day),"No",DR.Day),DR.Event=ifelse(is.na(DR.Event),"No",DR.Event))
...
Building<- Building_data |>
  ...
  dplyr::select(Timestamp, id, Date, Hour,Type,
                ...
                Ambient_Temperature_F)|> distinct()

# DR Schedule
DR_dates_times <- LMP_events |>
  dplyr::select(Date,event_days,end_date, starthour, endhour, DR_preevent)|>
  distinct()
...
baseline_energy<- Building_data |> 
  ...
  dplyr::select(Timestamp, Date, Hour,DR.Event,Type,
                id, Name,Gross.Square.Feet,Type,total_lmp_rt,
                "Electricity_Consumed_kWh","Electricity.outlier",
                "CW_elec","CW_elec.outlier",
                Building_Occupancy_DEV,
                Ambient_Temperature_F) |> distinct()
...
# write.table(baseline_energy,file="loads_shed_Energy_Market.csv", sep=",")
write.table(baseline_energy,file="loads_shed_Energy_200.csv", sep=",")
...
rf_Building_data<- left_join(rf_Building_data,LMP_events_join,relationship = "many-to-many")
rf_Building_data<- rf_Building_data|>
  mutate(DR.Day=ifelse(is.na(DR.Day),"No",DR.Day),DR.Event=ifelse(is.na(DR.Event),"No",DR.Event))
...
baseline_energy<-rf_Building_data|> 
  filter(DR.Day=="Yes") |> 
  dplyr::select(Timestamp, Date, Hour,DR.Event,Type,total_lmp_rt,
...
# write.table(baseline_energy,file="rf_CC_Energy.csv", sep=",")
# save.image("C:/Users/malhotra.164/OneDrive - The Ohio State University/Yashvi Malhotra-KWEST/BCA/R_BCA/R_BCA/Raw_data/Energy_market_baseline_image.RData")

write.table(baseline_energy,file="rf_200_Energy.csv", sep=",")
save.image("C:/Users/malhotra.164/OneDrive - The Ohio State University/Yashvi Malhotra-KWEST/BCA/R_BCA/R_BCA/Raw_data/Energy_market_200_image.RData")
