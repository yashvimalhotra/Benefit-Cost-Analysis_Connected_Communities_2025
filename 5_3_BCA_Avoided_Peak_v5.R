...
Building_data<- data_2019_22|>
  ...
  dplyr::select(id, Timestamp, Year, Date, Hour, Type, Actual_Cooling_Load_kWh,Steam_Consumed_kBTU)
...
# dont remove if there are no ECMs for that year
# Building data with ECM ---------------------------------------------------------------------
Building_data_ecm <- fuzzy_left_join(Building_data,ecm_savings_profile,
                                     by = c("id", "Date" = "End.Date"),match_fun = list(`==`, `>=`))

# Keep only the most recent (max) End.Date for each (id, Date)
Building_data_filtered <- Building_data_ecm |>
  dplyr::group_by(id.x, Date,Type) |> 
  ...
  rename(id=id.x)|>
  dplyr::select(id, Timestamp,Year,Date,Hour,End.Date, Type,
         Actual_Cooling_Load_kWh, EE_Cooling_kWh, ECM_Cooling_kWh,
         Steam_Consumed_mmBTU, EE_Heating_mmBTU, ECM_Heating_mmBTU)|>
  group_by(id, Timestamp,Year,Date,Hour,Type,
           Actual_Cooling_Load_kWh, EE_Cooling_kWh, ECM_Cooling_kWh,
           Steam_Consumed_mmBTU, EE_Heating_mmBTU, ECM_Heating_mmBTU) |>
  filter(if (all(is.na(End.Date))) TRUE else End.Date == max(End.Date, na.rm = TRUE))|>
  ungroup()

summary(Building_data_filtered)

Building_data_ecm_corrected<- Building_data_filtered |>
  group_by(Timestamp,Year, Date, Hour,Type)|>
...
S1_peak<-load_shed|>
  ...
  group_by(Year, Date, Hour,Type)|> # group all buildings
...
S5_peak <- load_shed |> 
  filter(DR.Event == "Yes") |>
  group_by(Year, Date, Hour, Market,Type)|> 
  summarise(Shed_GBM=sum(Shed_GBM, na.rm=T))|>
  group_by(Year, Date, Hour,Type)|> 
...
write.table(avoided_peak_summary, file="avoided_Type_peak.csv", sep=",", row.names = FALSE)
save.image("C:/Users/malhotra.164/OneDrive - The Ohio State University/Yashvi Malhotra-KWEST/BCA/R_BCA/R_BCA/Raw_data/Images/Avoided_Type_Peak.RData")
